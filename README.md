...
**Wo die Benutzer liegen:** in einer eigenen, privaten Tabelle `qc-review-users (privat)` unter `Claude Code/.credentials/qc-review/` (`CONFIG.USERS_SSID`). **Nicht** mehr als Tab im QUEUE-Spreadsheet — `sheet-60-qc-queue` liegt unter `full-ai-marketing-build` und ist an `bodo.buschick@exasync.ai` und ein exasync-Dienstkonto freigegeben (am 04.08.2026 über die Drive-API gemessen); Benutzerliste, Rollen, Salts und Hashes waren dort mitlesbar. `setup()` legt den Tab weiterhin dort an, wo `USERS_SSID` hinzeigt.
